...
def generate_dummy_data(
    ...
    """Generate a dummy price DataFrame for testing / demonstration."""
    ...
    for d in dates:
        for t in tickers:
            row: dict = {"date": d, "tic": t}
            row["close"] = abs(rng.normal(100, 20))
            row["volume"] = abs(rng.normal(1e6, 2e5))
            row["macd"] = rng.uniform(-2, 2)
            row["rsi"] = rng.uniform(20, 80)
            rows.append(row)
...
def main():
    ...
    n_assets = 5
    logger.info("Generating dummy market data (%d assets, 500 days) ...", n_assets)
    df = generate_dummy_data(n_days=500, n_assets=n_assets)

    paths = train_all_drl_agents(
        df,
        stock_dim=n_assets,
        total_timesteps=10_000,  # small for demo
        checkpoint_dir=CHECKPOINT_DIR,
    )
    logger.info("All models saved: %s", paths)
